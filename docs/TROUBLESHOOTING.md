...
**failed to load include path, android.jar, during aapt2 link.** Some SDK installs ship a preview platform whose `android.jar` that `aapt2` cannot load. The script prefers a stable `android-36` or the highest numeric `android-NN`. Check what you have with `ls $ANDROID_SDK_ROOT/platforms`.
...
Open an issue with the device model, Android version and manufacturer, whether you are Device Owner, the exact steps and what you expected against what you saw, and logcat from `adb logcat -s TL.Engine` if useful. Redact device identifiers.
